...
*/
    int index;    /**< stream index in AVFormatContext 在AVFormatContext中的stream索引 */
    /**
     * Format-specific stream ID.
     * decoding: set by libavformat
     * encoding: set by the user, replaced by libavformat if left unset
     */
    int id;  //格式特定的流的ID，解码的时候由libavformat设置，编码的时候用户设置，如果不设置libavformat会去设置
#if FF_API_LAVF_AVCTX
    /**
     * @deprecated use the codecpar struct instead
     */
    attribute_deprecated
    AVCodecContext *codec;  /*跟这个流相对应的编解码的上下文，由libavformat分配和释放，解码的时候：解封装器导出存储在头部中的编解码信息
    
编码的时候：用户设置codec信息，muxer将其写到输出*/
#endif
    void *priv_data;
     ...
     */
    AVRational time_base;//通过该值可以把PTS，DTS转化为真正的时间。FFMPEG其他结构体中也有这个字段，但是根据我的经验，只有AVStream中的time_base是可用的。PTS*time_base=真正的时间
...
typedef struct AVFormatContext {
     ...
     */
    ff_const59 struct AVInputFormat *iformat;//输入/输出格式，或为输入或为输出，这两个不能同时存在；分别用于编码和解码。AVOutputFormat类保存了文件名、扩展名、默认音视频和字幕解码器等信息
     ...
     */
    ff_const59 struct AVOutputFormat *oformat; //输出结构体
     ...
     */
    int64_t start_time; //媒体的开始时间 ，一般由AVStream **streams导出
     ...
     * Demuxing only, set by libavformat.
     */
    int64_t duration;//持续时长，一般由AVStream **streams导出
...
#define AVFMT_FLAG_AUTO_BSF   0x200000 ///< Add bitstream filters as requested by the muxer
     ...
     */
    int64_t probesize;   //在解码时用于探测的数据的大小，编码时不使用
     ...
     */
    int64_t max_analyze_duration; //允许输入数据在avformat_find_stream_info()中分析的最长时间
     ...
     * - demuxing: set by user
     */
    unsigned int max_index_size;  //检索码流的索引值的最大字长
...
#define AVFMT_AVOID_NEG_TS_MAKE_ZERO         2 ///< Shift timestamps so that they start at 0
     ...
     * - decoding: Set by user
     */
    int64_t skip_initial_bytes;   //打开媒体流时跳过的字节数
	...
	/*
	io_open函数用于建立和初始化一个AVIOContext结构体，用于打开指定的url指向的资源。各个参数的含义：

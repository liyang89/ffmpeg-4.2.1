...
typedef struct AVPacket {
     ...
     */
    AVBufferRef *buf;  //AVPacket内存引用计数
    /**
    ...
    int   size; //原始压缩数据的大小
    int   stream_index; //标识当前AVPacket所从属的码流,Packet所在stream的index
    /**
    ...
    int64_t pos;      ///< byte position in stream, -1 if unknown数据在媒体流中的位置，未知则值为-1
    ...
    int64_t convergence_duration;  //该字段已被deprecated，不再使用
#endif
} AVPacket;
...
typedef struct AVCodecContext {
    ...
    enum AVMediaType codec_type; /* see AVMEDIA_TYPE_xxx 编解码器的类型*/ 
    const struct AVCodec  *codec; //解码器结构体指针，在AVCodecContext创建之初就传进来。初始化后不可更改
    enum AVCodecID     codec_id; /* see AV_CODEC_ID_xxx 是什么类型的解码器*/
     ...
     */
    struct AVCodecInternal *internal; //内部使用的上下文，通常被用在libavcodec的函数中，并且不是解码器，指定区别与priv_data
     ...
     */
    int64_t bit_rate;  //平均比特率，编码时为用户设置。解码时也可以用户设置，但也可能被libavcodec覆盖
...
#define FF_COMPRESSION_DEFAULT -1
     ...
     *             overwrite those values as required while parsing the data.
     */
    int coded_width, coded_height; // 比特流宽度/高度可以与宽度/高度不同，例如 当在输出之前裁剪解码的帧或者启用lowres时。解码时使用，可能被解码器覆盖
     ...
     */
    int slice_count; //slice数,每帧视频切片数
...
#define FF_MB_DECISION_RD     2        ///< rate distortion
    ...
    int channels;    ///< number of audio channels  声道数
     ...
     * - decoding: Set by libavcodec.
     */
    enum AVSampleFormat sample_fmt;  ///< sample format  //音频采样格式
     ...
     */
    int (*get_buffer2)(struct AVCodecContext *s, AVFrame *frame, int flags); //在每个帧的开头调用此回调以获取它的数据缓冲区
     ...
     */
    attribute_deprecated
    int refcounted_frames; //通过vcodec_decode_video2()和avcodec_decode_audio4()调用返回的解码后的帧数
...
#define AV_EF_AGGRESSIVE (1<<18)        ///< consider things that a sane encoder should not do as an error
     ...
     * - decoding: Set by libavcodec
     */
    const struct AVHWAccel *hwaccel; // 正在使用的硬件加速器
     ...
     * - decoding: Set by user
     */
    void *hwaccel_context; //硬件加速器上下文 对于某些硬件加速器，用户需要提供全局上下文
...
#define FF_IDCT_SIMPLEAUTO    128
     ...
     */
    int thread_type;  // 编解码器正在使用哪种多线程方法
...
#define FF_LEVEL_UNKNOWN -99
     ...
     *             -initial_padding.
     */
    int initial_padding; // 仅限音频。 编码器在音频开头插入的“启动”样本（填充）的数量。即调用者必须丢弃此数量的前导解码样本，以获得原始音频而不带前导填充。
    ...
    int64_t pts_correction_num_faulty_dts; /// Number of incorrect DTS values so far 到目前为止，DTS值的错误数量
    int64_t pts_correction_last_pts;       /// PTS of the last frame 最后一帧的PTS
    int64_t pts_correction_last_dts;       /// DTS of the last frame 最后一帧的DTS
...
#define FF_SUB_TEXT_FMT_ASS_WITH_TIMINGS 1
     ...
     * - encoding: unused
     */
    int trailing_padding; // 仅限音频。 编码器附加到音频末尾的填充量（在样本中）。 即调用者必须从流的末尾丢弃此数量的解码样本，以获得原始音频而不进行任何尾随填充。
...
typedef struct AVCodec {
    ...
    enum AVMediaType type; // 媒体类型，有CODEC_ID_H264等
     ...
     */
    int capabilities;
    const AVRational *supported_framerates; //支持的帧率的数组，NULL表示任何的都支持，数组由{0,0}结束/< array of supported framerates, or NULL if any, array is terminated by {0,0}
    const enum AVPixelFormat *pix_fmts;     //支持的像素格式的数组，NULL表示未知，数组由-1结束（仅视频）/< array of supported pixel formats, or NULL if unknown, array is terminated by -1
    const int *supported_samplerates;       //supported_samples支持的声音采样频率的数组，NULL表示未知，数组由0结束/< array of supported audio samplerates, or NULL if unknown, array is terminated by 0
    const enum AVSampleFormat *sample_fmts; //sample_fmts支持的声音采样格式的数组，NULL表示未知，数组由-1结束/< array of supported sample formats, or NULL if unknown, array is terminated by -1
    const uint64_t *channel_layouts;         //支持的声音的通道数，NULL表示未知，由0结束/< array of support channel layouts, or NULL if unknown. array is terminated by 0
     ...
     *****************************************************************
     */
    int priv_data_size;  // 标示具体的Codec对应的Context的大小,如：H264Context
    struct AVCodec *next;
    /**
...
typedef struct AVCodecParameters {
     ...
     * bytes zeroed.
     */
    uint8_t *extradata;  //针对特定编码器包含的附加信息
    /**
